...
description: >
  ...
  data models, team/module/access-request architecture, and the Bob
  AI co-pilot integration. Activate this skill when working on any file
  in this repository.
triggers:
  - onboarding
  - access request
...
- **Bob AI co-pilot** is embedded in the running app as a chat widget
...
| `backend/bob_mcp_server.py` | MCP stdio server exposing 7 Bob tools |
...
| `frontend/src/components/BobCopilot.jsx` | Floating chat widget |
...
## Bob Co-pilot Integration
The chat integration was added as part of this hackathon submission:

### Backend (`backend/main.py` — `POST /bob/chat`)
...
- **To upgrade to a real LLM**: replace `_resolve_response()` body with a
  `watsonx.ai` or `openai.chat.completions.create()` call, passing the same
  live data as system context.

### Frontend (`frontend/src/components/BobCopilot.jsx`)
- Floating chat bubble (bottom-right, `position: fixed`)
- Renders markdown-lite (bold, italic, bullets)
- Page-aware: passes `pageContext` prop so Bob answers differently on admin vs engineer pages
- Added to `App.jsx` so it appears on every authenticated page

### MCP Server (`backend/bob_mcp_server.py`)
...
- Register in Bob IDE: `python backend/bob_mcp_server.py` as a local stdio MCP server
...
## Common Tasks for Bob (in IDE)
- "Add a new MCP tool that lists all teams" → add to `bob_mcp_server.py`'s `TOOLS` list and `call_tool()`
- "Add a `/bob/chat` intent for quiz results" → extend `_resolve_response()` in `main.py`
- "Style the chat panel to match IBM Carbon" → edit `BobCopilot.jsx`
- "Replace rule-based Bob with watsonx.ai" → swap `_resolve_response()` in `main.py`
